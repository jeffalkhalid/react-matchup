...
- [ ] 🔴 **Tester sur device réel** : login + signup (token accepté par Supabase). ⚠️ Si erreur de domaine, ajouter le hostname `icshhobxeppttgayxmba.supabase.co` dans la config Turnstile (déjà censé être ajouté).
