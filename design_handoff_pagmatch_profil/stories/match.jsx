...
function MatchScoreHero({ match: m }) {
  ...
  return (
    <div style={{ width: 1080, height: 1920, background: '#0A0A0A', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '110px 90px 96px', pointerEvents: 'none' }}>
      {window.StoryBg ? <window.StoryBg slotId="bg-match-hero" tone="dark" scrim="linear-gradient(180deg, rgba(10,10,10,0.55) 0%, rgba(10,10,10,0.62) 45%, rgba(10,10,10,0.85) 100%)" /> : null}
...
function MatchVersus({ match: m }) {
  return (
    <div style={{ width: 1080, height: 1920, position: 'relative', overflow: 'hidden', background: '#0A0A0A', pointerEvents: 'none' }}>
      {window.StoryBg ? <window.StoryBg slotId="bg-match-versus" tone="dark" scrim="rgba(10,10,10,0.32)" /> : null}
      <div className="pm-content" style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column' }}>
        {/* haut — vainqueurs */}
        <div style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 28, padding: '120px 80px 90px' }}>
          <div className="pm-base-wash" style={{ position: 'absolute', inset: 0, background: `linear-gradient(160deg, ${MC.brandDeep}, #1a1206)` }} />
          <div style={{ position: 'absolute', top: 96, left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 1 }}><BrandChip /></div>
        ...
        {/* bas — adversaires */}
        <div style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, padding: '90px 80px 130px' }}>
          <div className="pm-base-wash" style={{ position: 'absolute', inset: 0, background: 'linear-gradient(200deg, #1a1a1c, #0A0A0A)' }} />
...
function MatchTicket({ match: m }) {
  ...
  return (
    <div style={{ width: 1080, height: 1920, background: '#0A0A0A', display: 'grid', placeItems: 'center', position: 'relative', overflow: 'hidden', pointerEvents: 'none' }}>
      {window.StoryBg ? <window.StoryBg slotId="bg-match-ticket" tone="dark" scrim="rgba(10,10,10,0.42)" /> : null}
