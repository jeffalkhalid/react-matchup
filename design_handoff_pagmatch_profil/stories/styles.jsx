...
/* ── Fond photo optionnel (déposé par l'utilisateur) ──────────────────
   Couche la plus en arrière d'une story : <image-slot> + scrim. Vide → invisible
   (le fond d'origine du design reste) ; rempli → la photo couvre le fond. */
if (typeof document !== 'undefined' && !document.getElementById('pm-storybg-css')) {
  const sEl = document.createElement('style');
  sEl.id = 'pm-storybg-css';
  sEl.textContent =
    'image-slot.story-bg::part(frame){background:transparent}' +
    'image-slot.story-bg::part(ring){border-color:rgba(255,255,255,0.28)}' +
    'image-slot.story-bg-dark::part(empty){color:rgba(255,255,255,0.55)}' +
    'image-slot.story-bg-light::part(empty){color:rgba(10,10,10,0.4)}' +
    '.pm-base{transition:opacity .2s}' +
    'image-slot.story-bg[data-filled] ~ .pm-base{opacity:0}' +
    'image-slot.story-bg[data-filled] ~ .pm-content .pm-base{opacity:0}' +
    '.pm-base-wash{transition:opacity .2s}' +
    'image-slot.story-bg[data-filled] ~ .pm-base-wash{opacity:0.5}' +
    'image-slot.story-bg[data-filled] ~ .pm-content .pm-base-wash{opacity:0.5}';
  document.head.appendChild(sEl);
}
function StoryBg({ slotId, tone = 'dark', scrim }) {
  return (
    <React.Fragment>
      {React.createElement('image-slot', {
        id: slotId, fit: 'cover', shape: 'rect', class: 'story-bg story-bg-' + tone,
        placeholder: '📷 Photo de fond (optionnel)',
        style: { position: 'absolute', inset: 0, width: '1080px', height: '1920px', display: 'block', zIndex: 0, pointerEvents: 'auto' },
      })}
      {scrim ? <div style={{ position: 'absolute', inset: 0, background: scrim, pointerEvents: 'none', zIndex: 0 }} /> : null}
    </React.Fragment>
  );
}
...
function StoryCardDark({ player: p }) {
  ...
  return (
    <div style={{ width: 1080, height: 1920, background: '#0A0A0A', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', pointerEvents: 'none' }}>
      <StoryBg slotId="bg-profil-dark" tone="dark" scrim="linear-gradient(180deg, rgba(10,10,10,0.5) 0%, rgba(10,10,10,0.62) 45%, rgba(10,10,10,0.85) 100%)" />
      {/* glows */}
...
function StoryTradingCard({ player: p }) {
  ...
  return (
    <div style={{ width: 1080, height: 1920, background: '#0A0A0A', display: 'grid', placeItems: 'center', position: 'relative', overflow: 'hidden', pointerEvents: 'none' }}>
      <StoryBg slotId="bg-profil-trading" tone="dark" scrim="rgba(10,10,10,0.42)" />
...
function StoryEditorialLight({ player: p }) {
  ...
  return (
    <div style={{ width: 1080, height: 1920, position: 'relative', overflow: 'hidden', pointerEvents: 'none' }}>
      <StoryBg slotId="bg-profil-editorial" tone="light" />
      <div className="pm-base-wash" style={{ position: 'absolute', inset: 0, background: C.bgCream }} />
      <div className="pm-content" style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column', padding: '120px 92px 100px' }}>
      {/* top meta */}
...
function StoryCourt({ player: p }) {
  const gold = C.brand;
  return (
    <div style={{ width: 1080, height: 1920, position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg, #0B3B3B 0%, #072A2E 55%, #04181C 100%)', pointerEvents: 'none' }}>
      <StoryBg slotId="bg-profil-court" tone="dark" scrim="linear-gradient(180deg, rgba(4,16,18,0.55) 0%, rgba(4,16,18,0.5) 40%, rgba(4,16,18,0.82) 100%)" />
...
function StoryStatSheet({ player: p }) {
  ...
  return (
    <div style={{ width: 1080, height: 1920, background: '#0E1116', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '108px 80px 92px', pointerEvents: 'none' }}>
      <StoryBg slotId="bg-profil-stats" tone="dark" scrim="rgba(14,17,22,0.8)" />
...
Object.assign(window, { StoryFrame, StoryBg, STORY_STYLES, StoryCardDark, StoryTradingCard, StoryEditorialLight, StoryCourt, StoryStatSheet, PMRing: Ring, PMRadar: Radar, PMSparkline: Sparkline, PMQr: QRCode, PMInvite: Invite });
