...
function Composer({ open, mode, setMode, list, idx, setIdx, onClose, onShare, justSaved, matchData = window.PM.match }) {
  ...
  const addBg = () => {
    ...
    const slot = root && root.querySelector('image-slot.story-bg');
    if (slot && slot.shadowRoot) { const inp = slot.shadowRoot.querySelector('input[type="file"]'); if (inp) inp.click(); }
  };
  return (
    <div style={{
      ...
      {/* preview */}
      <div ref={previewRef} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', padding: '12px 0', minHeight: 0 }}>
        <div style={{ borderRadius: 18, overflow: 'hidden', boxShadow: '0 18px 50px rgba(0,0,0,0.22)', border: '1px solid rgba(0,0,0,0.06)' }}>
          <window.StoryFrame w={224} radius={18}><Cur player={PMP} match={matchData} slotId={'cmp-' + cur.id} /></window.StoryFrame>
        </div>
      </div>
      ...
      <div style={{ background: '#fff', borderTop: `1px solid ${PMC.bgCardAlt}`, paddingTop: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px 8px' }}>
          <span style={{ fontSize: 10, fontWeight: 900, color: PMC.textMuted, letterSpacing: 2 }}>STYLE{mode === 'photo' ? ' · DÉPOSE TA PHOTO' : ''}</span>
          {mode !== 'photo' ? <button onClick={addBg} style={{ display: 'flex', alignItems: 'center', gap: 5, border: `1px solid ${PMC.border}`, background: PMC.bg, borderRadius: 999, padding: '5px 11px', cursor: 'pointer', fontFamily: FF.ui, fontWeight: 800, fontSize: 11, color: PMC.textSecondary }}>📷 Photo de fond</button> : null}
        </div>
...
/* ── Share sheet iOS ──────────────────────────────────────────────── */
function ShareSheet({ open, Cur, curId, label, onClose, onPick, picked, matchData = window.PM.match }) {
  const apps = [
        ...
        <div style={{ background: 'rgba(250,250,250,0.96)', backdropFilter: 'blur(20px)', borderRadius: 22, overflow: 'hidden', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 14, borderBottom: '0.5px solid rgba(0,0,0,0.1)' }}>
            <div style={{ borderRadius: 8, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.18)' }}>
              <window.StoryFrame w={48} radius={8}><Cur player={PMP} match={matchData} slotId={'cmp-' + curId} /></window.StoryFrame>
            </div>
...
/* Exporté pour les profils retravaillés (réutilise le même composer + share sheet). */
Object.assign(window, { PMComposer: Composer, PMShareSheet: ShareSheet, PMChevron: Chevron });
