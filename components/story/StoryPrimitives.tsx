 ...
 * QR : utilise `react-native-qrcode-svg` (déjà installé).
...
// Wordmark officiel PagMatch (identité de marque, pas d'emoji raquette).
// Version claire (texte blanc) pour fonds sombres, foncée (texte noir) pour fonds clairs.
const WORDMARK_LIGHT = require('../../assets/auth/splash-wordmark.png');
const WORDMARK_DARK = require('../../assets/auth/splash-wordmark-dark.png');
const WORDMARK_RATIO = 364 / 105;
...
export function Wordmark({ s, light = true, h = 58 }: { s: (n: number) => number; light?: boolean; h?: number }) {
  return (
    <Image
      source={light ? WORDMARK_LIGHT : WORDMARK_DARK}
      style={{ height: s(h), width: s(h) * WORDMARK_RATIO }}
      resizeMode="contain"
    />
  );
}
