    ...
    <View style={{ flex: 1, padding: 14, justifyContent: 'center', gap: 8 }}>
      <View style={{ flexDirection: 'row', gap: 8 }}>
        {items.slice(0, 2).map((it, i) => (
          <View key={i} style={{ flex: 1, backgroundColor: '#fff', borderRadius: 14, padding: 12, alignItems: 'center', gap: 6, borderWidth: 1, borderColor: '#e2e8f0' }}>
            ...
            <Text style={{ fontSize: 10, fontWeight: '900', color: '#0f172a' }}>{it.label}</Text>
          </View>
        ))}
      </View>
      <View style={{ flexDirection: 'row', gap: 8 }}>
...
function IllustrationLobby() {
  ...
  return (
    <View style={{ flex: 1, padding: 12, justifyContent: 'center', gap: 8 }}>
      {cards.map((c, i) => (
        <View key={i} style={{ backgroundColor: '#fff', borderRadius: 14, borderWidth: 1.5, borderColor: c.border, padding: 12 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            ...
            <View style={{ backgroundColor: '#eef2ff', borderRadius: 99, paddingHorizontal: 8, paddingVertical: 3 }}>
              <Text style={{ fontSize: 9, fontWeight: '900', color: '#4f46e5' }}>1 place</Text>
            </View>
          </View>
          <Text style={{ fontSize: 13, fontWeight: '900', color: '#0f172a' }}>{c.loc}</Text>
        </View>
      ))}
      <View style={{ backgroundColor: '#4f46e5', borderRadius: 14, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        <Text style={{ fontSize: 18 }}>＋</Text>
        <View>
          <Text style={{ fontSize: 11, fontWeight: '900', color: '#fff' }}>Créer une partie</Text>
...
function IllustrationMatchmaking() {
  ...
  return (
    <View style={{ flex: 1, padding: 12, justifyContent: 'center', gap: 8 }}>
      {players.map((p, i) => (
        <View key={i} style={{ backgroundColor: '#fff', borderRadius: 14, borderWidth: 1, borderColor: `${p.color}33`, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 13, fontWeight: '900', color: '#0f172a' }}>{p.name}</Text>
            <Text style={{ fontSize: 9, color: '#64748b', marginTop: 1 }}>{p.level} · {p.label}</Text>
          </View>
          <View style={{ backgroundColor: p.color, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 6 }}>
            <Text style={{ fontSize: 10, fontWeight: '900', color: '#fff' }}>Défier</Text>
          </View>
        </View>
      ))}
    </View>
...
function IllustrationElo() {
  const leagues = [
    ...
    { icon: '🥈', label: 'Argent',     color: '#64748b', active: false, done: true },
  ...
  return (
    <View style={{ flex: 1, padding: 12, justifyContent: 'center', gap: 5 }}>
      <View style={{ backgroundColor: '#102820', borderRadius: 14, padding: 12, marginBottom: 6 }}>
        <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 9, fontWeight: '700', textTransform: 'uppercase' }}>Ton score ELO</Text>
        <Text style={{ color: '#fff', fontSize: 22, fontWeight: '900', marginTop: 2 }}>6.23</Text>
      ...
      {leagues.map((l, i) => (
        <View key={i} style={{
          ...
          {l.active && <View style={{ backgroundColor: l.color, borderRadius: 99, paddingHorizontal: 7, paddingVertical: 2 }}><Text style={{ fontSize: 8, fontWeight: '900', color: '#fff' }}>Actuel</Text></View>}
          {l.done && <Text style={{ fontSize: 12 }}>✅</Text>}
          {!l.done && !l.active && <Text style={{ fontSize: 11, color: '#94a3b8' }}>🔒</Text>}
        </View>
      ))}
    </View>
  );
...
function IllustrationChats() {
  ...
  return (
    <View style={{ flex: 1, overflow: 'hidden' }}>
      ...
      <View style={{ flex: 1, padding: 10, gap: 8, backgroundColor: '#f8fafc' }}>
        {msgs.map((m, i) => (
          <View key={i} style={{ flexDirection: 'row', justifyContent: m.mine ? 'flex-end' : 'flex-start' }}>
            <View style={{
              ...
              borderWidth: m.mine ? 0 : 1, borderColor: '#e2e8f0',
            }}>
              {!m.mine && <Text style={{ fontSize: 8, fontWeight: '700', color: '#94a3b8', marginBottom: 2 }}>{m.from}</Text>}
...
function IllustrationBadges() {
  const badges = [
    ...
    { icon: '🧱', label: 'Le Mur',     count: 7,  color: '#64748b', bg: '#f1f5f9' },
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
        {badges.map((b, i) => (
          <View key={i} style={{ width: (W - 24 - 6 * 3) / 3 - 1, backgroundColor: b.bg, borderRadius: 12, padding: 10, alignItems: 'center', gap: 4 }}>
            ...
            <Text style={{ fontSize: 9, color: '#64748b' }}>×{b.count}</Text>
          </View>
        ))}
      </View>
    </View>
      ...
      {items.map((it, i) => (
        <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: it.bg, borderRadius: 10, padding: 9 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 9, color: '#64748b', marginTop: 1 }}>{it.a}</Text>
          </View>
        </View>
      ))}
    </View>
...
export default function FeatureGuide() {
  ...
  return (
    <>
      ...
      >
        <Text style={{ color: '#fff', fontSize: 19, fontWeight: '900', lineHeight: 24 }}>?</Text>
      </TouchableOpacity>
        ...
        <View style={{ flex: 1, backgroundColor: '#fff' }} {...panResponder.panHandlers}>
          ...
          <Animated.View style={{ flex: 1, opacity: opacAnim, transform: [{ translateX: slideAnim }] }}>
            ...
            <View style={{ paddingHorizontal: 20, paddingTop: 14 }}>
              ...
              <Text style={{ fontSize: 14, fontWeight: '800', color: slide.accent, marginTop: 2 }}>
                {slide.subtitle}
              </Text>
            </View>
            ...
            >
              ...
              <Text style={{ fontSize: 13.5, color: '#475569', lineHeight: 20, fontWeight: '500' }}>
                {slide.tip}
              </Text>
            </ScrollView>
          ...
          <View style={{ paddingHorizontal: 20, paddingBottom: insets.bottom + 16, paddingTop: 10 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 10 }}>
              {index > 0 && (
                <TouchableOpacity
                  onPress={prev}
                  style={{ flex: 1, height: 52, borderRadius: 16, borderWidth: 1.5, borderColor: '#e2e8f0', alignItems: 'center', justifyContent: 'center' }}
                >
                  <Text style={{ fontSize: 14, fontWeight: '900', color: '#475569' }}>← Retour</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
                onPress={next}
            ...
            {!isFaq && (
              ...
              >
                <Text style={{ fontSize: 12, color: '#94a3b8', fontWeight: '600' }}>🆘 Un problème ? → Voir l'aide</Text>
              </TouchableOpacity>
            )}
          </View>
        </View>
