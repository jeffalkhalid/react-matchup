 ...
 *
 * Dépendances déjà présentes : react-native-view-shot, expo-image-picker, expo-sharing,
 * expo-media-library, react-native-safe-area-context. À installer : react-native-qrcode-svg. */
...
export default function StoryComposerV2({ visible, player, match, invite, onClose, onRequestMatch }: Props) {
  const insets = useSafeAreaInsets();
  const canvasRef = useRef<View>(null);
  const [mode, setMode] = useState<StoryMode>('profil');
  ...
  const switchMode = (m: StoryMode) => {
    ...
    if (m === 'photo' && !photoUri) pickPhoto(false);
  };
